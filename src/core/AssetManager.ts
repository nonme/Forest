...
export class AssetManager {
  ...
  public async loadAssets(
    ...
    const texturePromises = FOREST_OBJECTS_DATA.textures.map(async (texture) => {
      try {
        this.textures[texture.name] = await this.loadTexture(texture.path);
    ...
    const modelPromises = FOREST_OBJECTS_DATA.models.map(async (model) => {
      try {
        this.models[model.name] = await this.loadModel(model.path);
  ...
  private async getSkyboxFiles(folder: string): Promise<string[]> {
    // In a real scenario, you'd need to implement a way to get the list of files in the skybox folder.
    // For now, we'll return a dummy list.
    return ['SkySkybox.png', 'SunsetSky.png', 'BlueSkySkybox.png', 'FS000_Day_05_Sunless.png', 'FS002_Day_Sunless.png'];
  }
